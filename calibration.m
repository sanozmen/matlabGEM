...
%regarding the temperature dependence of Asat and phi(quantum yield)
   ...
   %//////////////////////////////////////////////////////////////////////////
  %/////////////////////////////////////////////////////////////////////////
Run=1;% first run of the model with 97 data to calibrate and obtain optimal values for 
%A_sat20 and Teta
...
PAR(PAR==-9999)=NaN;
PAR(PAR<0)=0;
PAR=PAR*1800;%PAR is recalculated to µmol/m2x30min
B=reshape(PAR,48,365); %each 48 (30 min) values make one day of the year
Daily_PAR=(sum(B)./(48*1800))'; %from µmol/m2d -> µmol/m2s-1
...
A_sat=A_sat20.*f_caAsat.*f_TAsat;
 ...
 A_stem=((phi.*I + A_sat - sqrt((phi.*I + A_sat).^2 - 4.*teta.*phi.*I.*A_sat))./2.*teta).*60*60*48*12/1000000;
...
A_stem=((phi.*I + A_sat - sqrt((phi.*I + A_sat).^2 - 4.*optimal_teta.*phi.*I.*A_sat))./2.*optimal_teta).*60*60*48*12/1000000;
...
%///////////////////////////////////////////////////////
%/////////////////////////////////////////////////
Run=2;% run of the model with 99 data and evaluate using 99 
...
Run=3;% run of the model with 01 data and evaluate using 01 
      %   data using parameters obtained in previuos CALIBRATION run
