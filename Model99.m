...
A_sat=A_sat20.*f_caAsat.*f_TAsat;
 ...
 A_stem=(phi.*I + A_sat - sqrt((phi.*I + A_sat).^2 - 4.*teta.*phi.*I.*A_sat))./(2.*teta).*(60*60*24*12/1000000);
...
A_stem=((phi.*I + A_sat - sqrt((phi.*I + A_sat).^2 - 4.*optimal_teta.*phi.*I.*A_sat))./2.*optimal_teta).*(60*60*24*12/1000000);
...
%-9999 data to NaN
PAR(PAR==-9999)=NaN;
PAR=PAR*1800; %µmol 30min
B=reshape(PAR,48,365);
Daily_PAR=(mean(B)/1800)';
