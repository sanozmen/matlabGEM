...
A_sat=A_sat20.*f_caAsat.*f_TAsat;
 ...
 A_stem=(phi.*Daily_PAR + A_sat - sqrt((phi.*Daily_PAR + A_sat).^2 - 4.*teta.*phi.*Daily_PAR.*A_sat))./2.*teta.*12;


Distance=sum((GPP(isfinite(GPP)& isfinite(A_stem))-(A_stem(isfinite(GPP)& isfinite(A_stem)))).^2); % Calculate the fit of the model to observed data
old_distance=Distance;

for teta=0.01 :0.01:1;
    for A_sat20=15.*86400/1000000:1:100.*86400/1000000;
        for phi15=0.001:0.001:0.01;
              if Daily_PAR==NaN
                 A_stem=NaN
              else
                 A_sat=A_sat20.*f_caAsat.*f_TAsat;
                 A_stem=(phi.*Daily_PAR + A_sat - sqrt((phi.*Daily_PAR + A_sat).^2 - 4.*teta.*phi.*Daily_PAR.*A_sat))./2.*teta.*12;
                 Distance=sum((GPP(isfinite(GPP)& isfinite(A_stem))-(A_stem(isfinite(GPP)& isfinite(A_stem)))).^2);
...
mean((GPP_s(isfinite(GPP_s)& isfinite(A_stem))-(A_stem(isfinite(GPP_s)& isfinite(A_stem)))).^2)
...
A_stem=((phi.*Daily_PAR + A_sat - sqrt((phi.*Daily_PAR + A_sat).^2 - 4.*optimal_teta.*phi.*Daily_PAR.*A_sat))./2.*optimal_teta).*12;
...
R2=corrcoef(GPP(isfinite(GPP)& isfinite(A_stem)), A_stem(isfinite(GPP)& isfinite(A_stem)))
figure(2)
plot(GPP(isfinite(GPP)& isfinite(A_stem)), A_stem(isfinite(GPP)& isfinite(A_stem)), 'o')
hold on
plot(GPP(isfinite(GPP)& isfinite(A_stem)), polyval(polyfit(GPP(isfinite(GPP)& isfinite(A_stem)), A_stem(isfinite(GPP)& isfinite(A_stem)),1),GPP(isfinite(GPP)& isfinite(A_stem))))
plot([0 14], [0 14], 'k.')
hold off